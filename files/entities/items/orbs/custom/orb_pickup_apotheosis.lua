	...
	if (orb_id > -1) then
		if( orb_id >= 100 ) then
			...
			if( GameGetOrbCollectedAllTime(orb_id) or GameIsDailyRunOrDailyPracticeRun() ) then
				...
				message_desc = "$orb_apotheosis_" .. orb_id_string .. "_desc"
				AddFlagPersistent( "apotheosis_card_unlocked_orb_" .. orb_id_string )
				AddFlagPersistent( "apotheosis_card_unlocked_orb_" .. orb_id_string .. "_spell" )
