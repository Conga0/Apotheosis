...
if GameGetFrameNum() >= cooldown_frame then
    if ComponentGetValue2(controlscomp, "mButtonDownRightClick") or InputIsJoystickButtonDown(0, 26) then
        ...
        if (mana > manacost) then
            ...
            GameShootProjectile(root, x+aim_x*12, y+aim_y*12, x+aim_x*20, y+aim_y*20, EntityLoad("data/entities/projectiles/deck/swapper.xml", x, y))
			if not HasFlagPersistent("action_apotheosis_alt_fire_swapper") then
				GameAddFlagRun("new_action_apotheosis_alt_fire_swapper")
				AddFlagPersistent("action_apotheosis_alt_fire_swapper")
			end
