...
if GameGetFrameNum() >= cooldown_frame then
	if isButtonDown_AltFire() then
		...
		if ModIsEnabled("quant.ew") then
			CrossCall("apoth_ew_alt_fire", root, x, y, aim_x, aim_y, "mods/Apotheosis/files/entities/projectiles/deck/markerportals/portal_green_marker.xml")
		end
	elseif ComponentGetValue2(controlscomp, "mButtonDownFire") and EntityGetName(EntityGetParent(entity_id))=="inventory_quick" then
		GameShootProjectile(root, x+aim_x*12, y+aim_y*12, x+aim_x*20, y+aim_y*20, EntityLoad("mods/Apotheosis/files/entities/projectiles/deck/markerportals/portal_green_portal.xml", x, y))
		ComponentSetValue2( variablecomp, "value_int", GameGetFrameNum() + cooldown_frames )
		if ModIsEnabled("quant.ew") then
			CrossCall("apoth_ew_alt_fire", root, x, y, aim_x, aim_y, "mods/Apotheosis/files/entities/projectiles/deck/markerportals/portal_green_portal.xml")
		end
	end
end