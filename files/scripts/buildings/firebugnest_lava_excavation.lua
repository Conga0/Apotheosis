		...
		if( distance_sqrt < spawn_distance * spawn_distance ) then
			...
			if (spawned_entities < 10) then
				...
				if (Random(1,10) < 9) then
					new_entity = EntityLoad( "data/entities/animals/lava_excavation/firebug.xml", pos_x, pos_y )
				else
					new_entity = EntityLoad( "data/entities/animals/lava_excavation/bigfirebug.xml", pos_x, pos_y )
				end
