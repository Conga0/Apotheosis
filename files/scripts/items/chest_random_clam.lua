...
function drop_random_reward( x, y, entity_id, rand_x, rand_y, set_rnd_  )
	...
	while( count > 0 ) do
		...
		if( rnd <= 7 ) then
			...
			-- Aqua Mine
			-------------------------------------------------------------------
			table.insert( entities, { "mods/Apotheosis/files/entities/projectiles/aquamine_slowhome.xml" } )
		...
		elseif( rnd <= 40 ) then
			...
			if( rnd > 30 and rnd <= 80 ) then
				chest_load_gold_entity( "mods/Apotheosis/files/entities/items/pickups/pearl_50.xml", x + Random(-10,10), y - 4 + Random(-10,5), remove_gold_timer )
			elseif (rnd <= 95) then
				chest_load_gold_entity( "mods/Apotheosis/files/entities/items/pickups/pearl_200.xml", x + Random(-10,10), y - 4 + Random(-10,5), remove_gold_timer )
			elseif (rnd <= 99) then
				chest_load_gold_entity( "mods/Apotheosis/files/entities/items/pickups/pearl_1000.xml", x + Random(-10,10), y - 4 + Random(-10,5), remove_gold_timer )
			else
			...
			for i=1,amount do
				chest_load_gold_entity( "mods/Apotheosis/files/entities/items/pickups/pearl.xml", x + Random(-10,10), y - 4 + Random(-10,5), remove_gold_timer )
			end
		elseif( rnd <= 50 ) then
			-------------------------------------------------------------------
			-- Potion
			...
			rnd = Random(0,100)
			if (rnd <= 94) then
				table.insert( entities, { "mods/Apotheosis/files/entities/items/pickups/potion_reinforced.xml" } )
			elseif (rnd <= 98) then
		...
		elseif( rnd <= 60 ) then
			...
			-- Misc items
			-------------------------------------------------------------------
			local opts = { "data/entities/items/pickup/safe_haven.xml", "data/entities/items/pickup/moon.xml", "data/entities/items/pickup/thunderstone.xml", "data/entities/items/pickup/evil_eye.xml", "data/entities/items/pickup/waterstone.xml", "runestone", "die", "orb" }
			rnd = Random( 1, #opts )
			local opt = opts[rnd]
			...
			elseif ( opt == "runestone" ) then
				local r_opts = { "laser", "fireball", "lava", "slow", "null", "disc", "metal", "alchemy" }
				rnd = Random( 1, #r_opts )
		...
		elseif( rnd <= 95 ) then
			-------------------------------------------------------------------
			-- Angler Perk (Not allowed if you don't have 1 perk already)... also an exceedingly rare chance for...
			-------------------------------------------------------------------
			
			rnd = Random(1,1000)
			if ( rnd == 1 ) then
				if ( not GameHasFlagRun("apotheosis_miniboss_boss_water_lukki") ) then
					GamePlaySound( "data/audio/Desktop/animals.bank", "animals/boss_centipede/destroy_face", x, y )
					table.insert( entities, { "data/entities/animals/boss_water_lukki/boss_fire_lukki.xml" } )
				else
					table.insert( entities, { "data/entities/animals/sunken_creature.xml" } )
				end
				good_item_dropped = false
			elseif (tonumber(GlobalsGetValue( "APOTHEOSIS_FISHING_SKILL", "0" )) > 0) then
				table.insert( entities, { "mods/Apotheosis/files/entities/items/pickups/perk_angler.xml" } )
			else
				rnd = Random(0,100)
		...
		elseif( rnd <= 98 ) then
			-------------------------------------------------------------------
			-- Converts the chest into red sand and rarely... something more divine...
			-------------------------------------------------------------------
			rnd = Random(1,100)
			if ( rnd == 1 ) then
				EntityConvertToMaterial( entity_id, "apotheosis_magic_liquid_divine")
			else
				EntityConvertToMaterial( entity_id, "apotheosis_redstone")
			end
		elseif( rnd <= 99 ) then
			-------------------------------------------------------------------
			-- exploding "dice"
