...
--[[
    ...
    },
    {
        ...
        trigger = function()
			...
			tone = "long"
		        if GameHasFlagRun("apotheosis_heretalk_cynical") then
            	            d_opts = {"It only works on stones.","Unlike you, we believe in failsafes."}
			    tone = "power"
		        end
                        local dialogue = d_opts[math.random(1,#d_opts)]
                        return true, dialogue
        ...
        trigger = function()
            ...
            for cd=1,#corrupt_death do
                if EntityGetName(corrupt_death[cd]) == "wizard_corrupt_death_check" then
                    local d_opts = {"They don't deserve this.", "How humane of you...", "They deserve better."}
		    if GameHasFlagRun("apotheosis_heretalk_cynical") then
            	        d_opts = {"Couldn't stop at me, could you?","They deserve better than this.","We were never meant to prosper, were we?"}
		    end
                    local dialogue = d_opts[math.random(1,#d_opts)]
	...
	--Will be changed
        trigger = function()
            ...
            currbiome = tostring(currbiome)
            local heretic_body = EntityGetFirstComponentIncludingDisabled(entity_id,"PhysicsBodyComponent")
            if GameHasFlagRun("apotheosis_heretalk_id_5") and EntityGetParent(entity_id) == 0 and ComponentGetIsEnabled( heretic_body ) == true then
		local timer = 0
		...
		if (timer == 600) then
            	    local d_opts = {"I've been sitting here for 10 seconds."}
            	    local dialogue = d_opts[math.random(1,#d_opts)]
		    tone = "gossip"
            	    return true, dialogue
		end
	...
	--Will be changed
		...
		timer = ComponentGetValue2( timercomp, "value_float")
		if (timer == 3600) then
            	    local d_opts = {"I've been sitting here for a minute."}
            	    local dialogue = d_opts[math.random(1,#d_opts)]
		    tone = "gossip"
            	    return true, dialogue
		end
