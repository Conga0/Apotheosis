...
function damage_received( damage, desc, entity_who_caused, is_fatal )
	...
	if phase == 1 and ((max_health * 0.4) >= health) then
		...
		--
		--local cid = EntityLoad( "data/entities/animals/boss_water_lukki/gore/cell_eater_toggle.xml", pos_x, pos_y )
		...
		local lukki_new = EntityLoad("data/entities/animals/boss_water_lukki/boss_fire_lukki_phase2.xml", pos_x, pos_y)


		--Triggers the mass Death Cross attack upon transformation
		local cid = EntityLoad( "data/entities/animals/boss_water_lukki/projectiles/deathcross_barrage_setup_long.xml", pos_x, pos_y )
		EntityAddChild( lukki_new, cid )
		EntityLoad( "data/entities/animals/boss_water_lukki/projectiles/circle_explosion.xml", pos_x, pos_y )
		...
		EntityAddComponent2(
			...
			"LuaComponent",
			{
				...
				script_source_file = "data/entities/animals/boss_water_lukki/boss_fire_lukki_deathcross_barrage_setup.lua",
		...
		EntityAddComponent2(
			...
			{
				execute_on_added = false,
				script_source_file = "data/entities/animals/boss_water_lukki/boss_fire_lukki_healing_phase_logic_lowhp_burst.lua",
	...
	if phase == 2 and ((max_health * 0.1) >= health) then
		...
		local cid = EntityLoad( "data/entities/animals/boss_water_lukki/misc/healing_phase_helper.xml", pos_x, pos_y )
		EntityAddChild( entity_id, cid )
		...
		if ( storages ~= nil ) then
			for i,comp in ipairs( storages ) do
				...
				if ( name == "data/entities/animals/boss_water_lukki/boss_fire_lukki_healing_phase_logic.lua" ) then
	...
	if phase == 4 then
		...
		while (nearest_interval > new_health) and (count < limit) and (nearest_interval >= 0) do
			local eid = EntityLoad( "data/entities/animals/boss_water_lukki/misc/healing_phase_helper.xml", pos_x, pos_y )
