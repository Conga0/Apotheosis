...
deadly_locations = {
    {-16268,7908}, --Sunken Cavern
    {16200,9900}, --Power Plant
    {9740,9130}, --The Tower
    {2740,11550}, --Temple of the Art
    {-8960,14650} --Bottom of the Snowy Chasm
}
...
function stage3()
    if teleported then
        GameSetPostFxParameter("conga_constellation_transition_amount", math.min(1,2.6 - ((frame - init_frame) / 200)), 0, 0, 0)
    else
    ...
    if frame == init_frame + 300 then
        ...
        if phaseout_stage > 202 then
            ...
            local rng = Random(1,#deadly_locations)
            local tele_x = deadly_locations[rng][1]
            local tele_y = deadly_locations[rng][2]
            EntitySetTransform(parent_id,tele_x,tele_y)
